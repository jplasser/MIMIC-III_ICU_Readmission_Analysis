...
train_raw = pickle.load(open(os.path.join(path, 'train_data'), 'rb'))
val_raw = pickle.load(open(os.path.join(path, 'val_data'), 'rb'))

if args.mode == 'train':

    # Prepare training
    path = 'keras_states_mimic3/' + model.final_name + '.epoch{epoch}.test{val_loss}.state'
    ...
    if not os.path.exists('keras_logs_mimic3'):
        os.makedirs('keras_logs_mimic3')
    csv_logger = CSVLogger(os.path.join('keras_logs_mimic3', model.final_name + '.csv'),
    ...
    ret = pickle.load(open(os.path.join(path, 'test_data'), 'rb'))
