...
def assemble_episodic_data(stays, diagnoses):
    ...
    data = { 'Icustay': stays.stay_id, 'Age': stays.age, 'Length of Stay': stays.los,'Mortality': stays.mortality0,'Transfer Back': stays.transferback, 'Die in Ward': stays.dieinward, 'Readmit within 30 days': stays.less_tahn_30days, 'Die within 30 days': stays.die_less_tahn_30days ,'Readmission': stays.readmission }
    ...
    data = data[['Ethnicity', 'Gender', 'Age', 'Height', 'Weight', 'Length of Stay', 'Mortality', 'Transfer Back', 'Die in Ward', 'Readmit within 30 days', 'Die within 30 days' ,'Readmission']]
...
def extract_diagnosis_labels(diagnoses):
    ...
    diagnoses['value'] = 1
    labels = diagnoses[['stay_id', 'icd_code', 'value']].drop_duplicates().pivot(index='stay_id', columns='icd_code', values='value').fillna(0).astype(int)
...
def make_phenotype_label_matrix(phenotypes, stays=None):
    ...
    phenotypes = phenotypes[['stay_id', 'hcup_ccs_2015']].ix[phenotypes.use_in_benchmark > 0].drop_duplicates()
    phenotypes['value'] = 1
    phenotypes = phenotypes.pivot(index='stay_id', columns='hcup_ccs_2015', values='value')
    if stays is not None:
        phenotypes = phenotypes.ix[stays.stay_id.sort_values()]
