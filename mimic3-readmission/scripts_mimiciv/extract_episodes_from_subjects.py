...
from mimic4benchmark.subject import read_stays, read_diagnoses, read_events, get_events_for_stay, add_hours_elpased_to_events
from mimic4benchmark.subject import convert_events_to_timeseries, get_first_valid_from_timeseries
from mimic4benchmark.preprocessing import read_itemid_to_variable_map, map_itemids_to_variables, read_variable_ranges, clean_events
from mimic4benchmark.preprocessing import transform_gender, transform_ethnicity, assemble_episodic_data
...
for subject_dir in os.listdir(args.subjects_root_path):
    ...
    if len(episodic_data.index) != 0:
        ...
        for i in range(stays.shape[0]):
            ...
            episode.to_csv(os.path.join(args.subjects_root_path, subject_dir, 'episode{}_timeseries_readmission.csv'.format(i+1)), index_label='Hours')
        sys.stdout.write(' DONE!\n')
    else:
        sys.stdout.write(' -> episodic data is empty, DONE!\n')
